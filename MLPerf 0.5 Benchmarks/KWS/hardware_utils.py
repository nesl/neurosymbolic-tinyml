...
def convert_to_cpp(model,model_name,quantization=True):
    convert_to_tflite(model,model_name)
    if(quantization==True):
        os.system("xxd -i trained_models/"+ model_name + "_quant.tflite > kws_model_data.cc")
        with open('kws_model_data.cc') as f:
            z = f.readlines()
        f.close()   
        z.insert(0,'#include "kws_model_data.h"\n')
        z = [w.replace('unsigned char','const unsigned char') for w in z]
        z = [w.replace('trained_models_g_kws_model_data_quant_tflite','g_kws_model_data') for w in z]
        z = [w.replace('unsigned int','const unsigned int') for w in z]
        my_f = open("kws_model_data.cc","w")
        for item in z:
            my_f.write(item)
        my_f.close()
    else:
        os.system("xxd -i trained_models/"+ model_name + ".tflite > kws_model_data.cc")
        with open('kws_model_data.cc') as f:
            z = f.readlines()
        f.close()   
        z.insert(0,'#include "kws_model_data.h"\n')
        z = [w.replace('unsigned char','const unsigned char') for w in z]
        z = [w.replace('trained_models_g_kws_model_data_tflite','g_kws_model_data') for w in z]
        z = [w.replace('unsigned int','const unsigned int') for w in z]
        my_f = open("kws_model_data.cc","w")
        for item in z:
            my_f.write(item)
...
def platform_in_the_loop_controller(model,model_name, hardware,dir_path='KWS_Mbed_Prog/',quantization=True):
    err_flag = 0
    RAM = -1
    Flash = -1
    err_flag = -1
    ...
    shutil.copy('kws_model_data.cc',dir_path+'kws/kws_model_data.cc')
    ...
    with cd(dir_path):
        ...
        if(quantization==True):
            ...
            z[z.index([i for i in z if 'input_asint[kKwsInputSize]' in i][0])] = 'int8_t input_asint[kKwsInputSize];\n';
