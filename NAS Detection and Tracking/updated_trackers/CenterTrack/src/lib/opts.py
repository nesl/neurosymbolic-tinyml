...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument("--no_train", action='store_true')
    ...
    self.parser.add_argument('--actual_num_stacks', default=1)
    self.parser.add_argument('--kernel_size', default=1)
    self.parser.add_argument('--head_conv_value', default=256)
    self.parser.add_argument('--activations', default=True)
...
#       opt.new_thresh = max(opt.track_thresh, opt.new_thresh)
    ...
    if opt.resume and opt.load_model == '':
      ...
      # opt.load_model = os.path.join(opt.save_dir, 'mo17_half.pth')
    return opt
