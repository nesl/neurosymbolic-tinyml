...
class Detector(object):
  def __init__(self, opt):
    ...
    with open("model_config.pkl", "rb") as d:
        model_config = pickle.load(d)

    self.model = create_model(
      opt.arch, opt.heads, opt.head_conv, model_config["num_stacks"], model_config["kernel_size"], \
        model_config["activations"], model_config["head_conv_value"], opt=opt)
