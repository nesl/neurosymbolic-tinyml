...
def create_model(arch, head, head_conv, actual_num_stacks, kernel_size, activations, head_conv_value, opt=None):
  num_layers = int(arch[arch.find('_') + 1:]) if '_' in arch else 0
  arch = arch[:arch.find('_')] if '_' in arch else arch
  model_class = _network_factory[arch]
  print(opt)
  # model = model_class(num_layers, heads=head, head_convs=head_conv, actual_num_stacks, kernel_size, activations, head_conv_value, opt=opt)
  model = DLASeg(num_layers, head, head_conv, actual_num_stacks, kernel_size, activations, head_conv_value, opt=opt)
  # model = PoseResDCN(num_layers, head, head_conv, actual_num_stacks, kernel_size, activations, head_conv_value, opt=opt)
...
def save_model(path, epoch, model, optimizer=None):
  ...
  # path = "../models/model_last.pth"  this is already saved in main
